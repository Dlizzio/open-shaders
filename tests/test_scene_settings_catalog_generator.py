...
class SceneSettingsCatalogGeneratorTests(unittest.TestCase):
    ...
    def test_reversible_numeric_transform_uses_storage_domain_bounds(self):
        exposure = self.entries_by_id[
            ("PostProcessing",
             "Color Grading and Tone Mapping/settings/exposureTemperatureTint", "x")]
        self.assertEqual(exposure["numericTransform"], "Log2")
        self.assertEqual((exposure["minimum"], exposure["maximum"]), (0.0625, 16.0))
    ...
    def test_choice_without_one_control_label_stays_unlabeled(self):
        ...
        self.assertEqual(selection["editorSemantic"], "Choice")
        self.assertEqual((selection["displayName"], selection["displayNameKey"]), ("", ""))
    ...
    def test_persisted_vectors_and_arrays_are_cataloged_as_scalar_components(self):
        ...
        self.assertTrue(any(entry["serializedKey"].isdigit() for entry in vector_entries))
