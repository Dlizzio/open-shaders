...
void WeatherPicker::DrawWeatherStatusPanel()
{
	...
	auto* lastWeather = sky && sky->lastWeather ? sky->lastWeather : s_cachedLastWeather;
	const float lerpFactor = sky && std::isfinite(sky->currentWeatherPct) ?
	...
	if (currentWeather) {
		...
		if (isTransitioning) {
			...
			ImGui::ProgressBar(lerpFactor, ImVec2(-1, 0), transitionOverlay.c_str());
		} else {
